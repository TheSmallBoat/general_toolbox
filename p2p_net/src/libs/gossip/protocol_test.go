...
import (
	...
	"awesomeProject/beacon/p2p_network/libs/gossip"
...
func TestGossip(t *testing.T) {
	...
	nodes := make([]*core_module.Node, 0, 16)
	overlays := make([]*kademlia.Protocol, 0, cap(nodes))

	seen := make(map[cryptographic.PublicKey]struct{}, cap(nodes))
	...
	for i := 0; i < cap(nodes); i++ {
		...
		hub := gossip.New(overlay,
			gossip.WithEvents(
				gossip.Events{
					OnGossipReceived: func(sender cryptographic.ID, data []byte) error {
						cond.L.Lock()
						seen[node.ID().PubKey] = struct{}{}
						cond.Signal()
						cond.L.Unlock()
	...
	leader, err := core_module.NewNode()
	assert.NoError(t, err)
	defer leader.Close()
