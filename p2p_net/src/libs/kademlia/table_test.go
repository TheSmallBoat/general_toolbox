...
func TestDistanceMetric(t *testing.T) {
	...
	ids := make([]cryptographic.ID, 0, 128)

	// Create a bunch of random IDs.

	for i := 0; i < cap(ids); i++ {
		pub, _, err := cryptographic.GenerateKeys(nil)
		...
		id := cryptographic.NewID(pub, host, port)
	...
	seen := make(map[cryptographic.PublicKey]struct{}, cap(ids))

	for _, table := range tables {
		for _, id := range table.Peers() {
			seen[id.PubKey] = struct{}{}
		}
	}
