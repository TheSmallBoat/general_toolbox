...
func main() {
	...
	logger, err := zap.NewDevelopment(zap.AddStacktrace(zap.InfoLevel))
	if err != nil {
		panic(err)
	}
	defer logger.Sync()
	...
	// Command line : ./chat -h 127.0.0.1 -p 9000 ":9000"
	node, err := core_module.NewNode(
		core_module.WithNodeLogger(logger),
		core_module.WithNodeBindHost(*hostFlag),
		core_module.WithNodeBindPort(*portFlag),
		core_module.WithNodeAddress(*addressFlag),
	)
	check(err)
	...
	events := kademlia.Events{
		OnPeerAdmitted: func(id cryptographic.ID) {
			fmt.Printf("Learned about a new peer %s(%s).\n", id.Address, id.PubKey.String()[:printedLength])
		},
...
func handle(ctx core_module.HandlerContext) error {
	if ctx.IsRequest() {
		return nil
	}
	...
	fmt.Printf("%s(%s)> %s\n", ctx.ID().Address, ctx.ID().PubKey.String()[:printedLength], msg.contents)

	return nil
}

// help prints out the users ID and commands available.
func help(node *core_module.Node) {
...
func chat(node *core_module.Node, overlay *kademlia.Protocol, line string) {
	switch line {
	case "/discover":
		discover(overlay)
		return
	...
	for _, id := range overlay.Table().Peers() {
		...
		if err != nil {
			fmt.Printf("Failed to send message to %s(%s). Skipping... [error: %s]\n",
				id.Address,
				id.PubKey.String()[:printedLength],
				err,
			)
			continue
		}
