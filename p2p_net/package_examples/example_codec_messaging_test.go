...
// ChatMessage is an example struct that is registered on example nodes, and serialized/de-serialized on-the-fly.
...
// This example demonstrates messaging with registering Go types to be serialized/de-serialized on-the-wire provided
...
func Example_codecMessaging() {
	...
	alice, err := core_module.NewNode()
	if err != nil {
		panic(err)
	}

	bob, err := core_module.NewNode()
	if err != nil {
		panic(err)
	}
	...
	alice.Handle(func(ctx core_module.HandlerContext) error {
	...
	bob.Handle(func(ctx core_module.HandlerContext) error {
		obj, err := ctx.DecodeMessage()
		if err != nil {
			return nil
		}
